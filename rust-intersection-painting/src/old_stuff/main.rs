...
async fn execute_compute_shader_once_gpustate(gpustate: &state::GpuState) -> Option<Vec<f32>>{
    ...
    let texture_bind_group = gpustate.device.create_bind_group(
        &wgpu::BindGroupDescriptor {
            layout: &gpustate.compute_bind_group_layout,
            entries: &[
                wgpu::BindGroupEntry {
                    binding: 0,
                    resource: wgpu::BindingResource::TextureView(&gpustate.grid_texture_view),
                },
                wgpu::BindGroupEntry {
                    binding: 1,
                    resource: wgpu::BindingResource::TextureView(&gpustate.line_texture_view),
                },
                wgpu::BindGroupEntry {
                    binding: 2,
                    resource: wgpu::BindingResource::Sampler(&gpustate.texture_sampler),
                },
                wgpu::BindGroupEntry {
                    binding: 3,
...
async fn execute_display_shader_once(num_segments: u32, gpustate: &state::GpuState, colours: Vec<f32>){
    ...
    let texture = gpustate.device.create_texture(&texture_desc);
    ...
    let output_buffer = gpustate.device.create_buffer(&output_buffer_desc);
}